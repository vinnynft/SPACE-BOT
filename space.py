...
str_in = """
    ...
    +++++++++++++++++++++++++++++++++++++++++++++++
    +++ Se te ajudou e quiser retribuir... ++++++++
    ++++++++++++ SPG SPE BCOIN BUSD BNB +++++++++++
    + 0x4e69D63FeD6d7F146D5ADA2499A1108A6e3fBE5A ++
    +++++++++++++++++++++++++++++++++++++++++++++++
    

    >> Ctrl + c finaliza o bot.

    >> Tela do jogo bem pequena (22,5cm / 17,5 cm)
...
def confirm():
    ...
    if clickBtn(images['confirm'], name='okBtn', timeout=3):
        dbg.console('Confirmado','INFO', 'ambos')
        time.sleep(2) 
        endFight()  
        confirm_action = True
    if clickBtn(images['confirm-victory'], name='okVicBtn', timeout=1):
        dbg.console('Proximo Boss...','INFO', 'ambos')
        confirm_action = True
...
def clickButtonsFight():
    ...
    for (x, y, w, h) in buttons:
        ...
        if ships_clicks >= qtd_send_spaceships:
            dbg.console('Esquadrão Pronto, BORA', 'INFO', 'ambos')
            return -1
    return len(buttons)
...
def screen_close():
    if clickBtn(images['close']):
        dbg.console('Fechando', 'ERROR', 'ambos')
        return True
    else:
        return False
...
def refreshSpaceships(qtd):
    ...
    dbg.console('Mandando as Naves para a Guerra', 'INFO', 'ambos')
    buttonsClicked = 1
    cda =  100
    ...
    while(empty_scrolls_attempts >0):
        ...
        time.sleep(2)
        dbg.console('Nave indo para a Peleia: ' + str(ships_clicks), 'INFO', 'ambos')
...
def endFight():
    dbg.console("Fim da Peleia", 'INFO', 'ambos')
    time.sleep(3) 
    returnBase()
    time.sleep(15) 
    if len(positions(images['spg-processing'], 0.9)) > 0:
...
def main():
    ...
    print(str_in)        
    time.sleep(5)
    dbg.console('BOT ON, Durma Bem :D. Versao: ' + str(VERSAO_SCRIPT), 'INFO', 'ambos')
    ...
    while True:
        ...
        if action_found == False:
            dbg.console('Tudo Certo... Segue o Farm', 'WARNING', 'ambos')
